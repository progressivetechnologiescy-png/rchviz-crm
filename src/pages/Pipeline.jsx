    ...
    <div className="pipeline-col">
        <div className="col-header">
            <div className="col-title-wrap">
                ...
                <h3 className="col-title">{toGreekCaps(t(`pipeline_stage_${column.title.toLowerCase().replace(' ', '_')}`, column.title))}</h3>
    ...
    return (
        ...
        >
            <header className="page-header">
                <div>
                    <h1 className="page-title">{t('sales_pipeline', 'Sales Pipeline')}</h1>
                    <p className="page-subtitle">{t('track_leads_opportunities', 'Track leads and opportunities.')}</p>
                </div>
                <div className="header-actions">
                    ...
                    <button className="btn btn-primary" onClick={() => setIsModalOpen(true)}>
                        <Plus size={16} />
                        <span>{t('add_lead', 'Add Lead')}</span>
                    </button>
                </div >
            </header >

            <motion.section className="stats-grid" variants={containerVariants} initial="hidden" animate="show" style={{ marginBottom: '2rem' }}>
                <StatCard title={t('total_leads', 'Total Leads')} value={pipelineData ? Object.values(pipelineData.columns).reduce((acc, col) => acc + col.taskIds.length, 0) : 0} detail={t('all_active_stages', 'All active stages')} icon={Users} trend="primary" />
                <StatCard title={t('new_leads', 'New Leads')} value={pipelineData?.columns['column-1']?.taskIds?.length || 0} detail={t('awaiting_contact', 'Awaiting contact')} icon={Clock} trend="warning" />
                <StatCard title={t('proposals', 'Proposals')} value={pipelineData?.columns['column-3']?.taskIds?.length || 0} detail={t('pending_approval', 'Pending approval')} icon={TrendingUp} trend="up" />
                <StatCard title={t('won', 'Won')} value={pipelineData?.columns['column-4']?.taskIds?.length || 0} detail={t('successfully_closed', 'Successfully closed')} icon={CheckCircle} trend="success" />
            </motion.section >
            ...
            <div className="board-container">
                <DragDropContext onDragEnd={onDragEnd}>
                    {pipelineData?.columnOrder?.map(columnId => {
                        ...
                        return <PipelineColumn key={column.id} column={column} leads={tempLeads} t={t} />;
                    })}
                </DragDropContext>
            </div>
