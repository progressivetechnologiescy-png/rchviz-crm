...
const Dashboard = () => {
    ...
    let maxCount = 0;
    chartData.forEach(d => {
        if (d.name !== 'Completed' && d.count > maxCount) {
            maxCount = d.count;
            mostActivePhase = d.name;
        }
    });
        ...
        return (
            ...
            >
                <div className="section-header">
                    ...
                    <Link to="/tasks" className="btn btn-ghost btn-sm no-underline text-inherit hover:text-[var(--accent-cyan)]">View All</Link>
                </div>
                            ...
                            >
                                ...
                                <div className="widget-task-actions flex items-center justify-end gap-2 absolute right-3 top-1/2 -translate-y-1/2">
                                    ...
                                    {task.priority === 'High' && task.status !== 'Completed' && (
                                        <span className="widget-task-urgent">Urgent</span>
                                    )}
    ...
    const renderAdminStats = () => {
        const statsData = {
            ...
            'total-revenue': { id: 'total-revenue', title: "Total Revenue", value: <span style={{ color: 'var(--status-success)' }}>€{totalRevenue.toLocaleString()}</span>, detail: "YTD Collected", icon: Euro, trend: "success" },
            'outstanding': { id: 'outstanding', title: "Outstanding", value: <span style={{ color: 'var(--status-danger)' }}>€{outstandingBalance.toLocaleString()}</span>, detail: "Needs collection", icon: Wallet, trend: "warning" }
        };
        ...
        <section className="recent-projects glass-panel w-full h-full">
            <div className="section-header">
                ...
                <div className="flex gap-3 text-sm">
                    ...
                    {userRole !== 'client' && (
                        <Link to="/production" className="btn btn-ghost btn-sm no-underline text-inherit hover:text-[var(--text-primary)]">View All</Link>
                    )}
                </div>
            </div>
        ...
        >
            <header className="page-header">
                <div>
                    ...
                    <p className="page-subtitle mb-2">Welcome back, {currentUser?.name || 'User'}</p>
                    {userRole === 'admin' && (
                        <button
